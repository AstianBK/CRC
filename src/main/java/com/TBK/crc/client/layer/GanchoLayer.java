...
public class GanchoLayer<T extends LivingEntity,M extends EntityModel<T>> extends RenderLayer<T,M> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(CRC.MODID,"textures/hand/cyborgarm.png");

    public GanchoModel model;
    public GanchoLayer(RenderLayerParent<T, M> p_117346_) {
        super(p_117346_);
    ...
    public void render(PoseStack p_117349_, MultiBufferSource p_117350_, int p_117351_, T p_117352_, float p_117353_, float p_117354_, float p_117355_, float p_117356_, float p_117357_, float p_117358_) {
        ...
        if(cap!=null){
            if (cap.isCatchedTarget(p_117352_)){
                p_117349_.pushPose();
                float f0 = Mth.lerp(p_117355_,p_117352_.yRotO,p_117352_.getYRot());
                float f1 = Mth.lerp(p_117355_,p_117352_.xRotO,p_117352_.getXRot());
                p_117349_.mulPose(Axis.YP.rotationDegrees(f0 - 180.0F));
                p_117349_.mulPose(Axis.ZP.rotationDegrees(f1));
                p_117349_.translate(CRC.x,CRC.y ,CRC.z);
                this.model.renderToBuffer(p_117349_,p_117350_.getBuffer(RenderType.entityCutoutNoCull(TEXTURE)), p_117351_,OverlayTexture.NO_OVERLAY,1.0f,1.0f,1.0f,1.0f);

                p_117349_.popPose();

                renderLeash((T) p_117352_,p_117355_,p_117349_,p_117350_,player);
            }
        }
    }
    ...
    private static void addVertexPair(LivingEntity p_115462_,VertexConsumer p_174308_, Matrix4f p_254405_, float p_174310_, float p_174311_, float p_174312_, int p_174313_, int p_174314_, int p_174315_, int p_174316_, float p_174317_, float p_174318_, float p_174319_, float p_174320_, int p_174321_, boolean p_174322_) {
        ...
        if(p_174321_ == (p_115462_.tickCount%25)){
            f3 *= 1.1F;
            f4 *= 1.1F;
        }
    ...
    protected int getBlockLightLevelForEntity(Entity p_114496_, BlockPos p_114497_) {
